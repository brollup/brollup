![Brollup](https://i.ibb.co/tc7S2JL/brollup-github.png)
Brollup is a Bitcoin-native rollup designed for payments and smart contracts at scale. It utilizes Bitcoin block space for data availability and integrates a state channel model with global state. Operating in a layer-two setting, Brollup supports unilateral exits and requires no trust assumptions.

|Competitive Landscape |  Trust Model | Protocol Changes?
|:------------|:-------------------------------------------------------|:--------|
| BitVM       | Permissioned Bitcoin peg with 1-of-n trust assumption. | -       |
| CatVM       | Permissioned Bitcoin peg with 1-of-n trust assumption. | BIP0420 |
| Sovereign   | No Bitcoin peg.                                        | -       |
| Brollup     | Trustless Bitcoin peg with unilateral exit.            | -       |

> [!NOTE]
> Work in progress.